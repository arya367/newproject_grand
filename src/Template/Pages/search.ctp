<?  use Cake\Datasource\ConnectionManager; $connection = ConnectionManager::get('default'); if(isset($total) and $total!=0) { ?>
<?  foreach($results as $data) {?>
<div class="row pr_list">
<div class="col-lg-4 col-md-4 col-sm-6 col-xs-4 col-xsmall"><div class="pr_img"><img src="<? echo SITE_PATH.'img/property/propertyimage/'.$data['property_image']; ?>" class="img-responsive" alt="<? echo $data['name'] ; ?>"></div></div>
<div class="col-lg-8 col-md-8 col-sm-6 col-xs-8 col-xsmall"><div class="pr_det">
<div class="row">
<div class="col-lg-6 col-md-6 col-xs-7 col-xsmall"><div class="pr_heading"><h3><a href="<? echo SITE_PATH.'project/'.$data['url']; ?>/"><? echo $data['name'] ; ?></a></h3><div class="locality"><i class="fa fa-map-marker"></i> <?= strlen($data['sectorurl'])>4 ? $this->Html->link($data['location_text'], SITE_PATH.'location/'.$data['locaturl'].'/'.$data['sectorurl']) : $data['location_text'] ?> <? echo ' ( '.ucwords($data['locatname']).' ) ' ; ?></div></div></div>
<div class="col-lg-6 col-md-6 col-xs-5 col-xsmall"><div class="price"><i class="fa fa-rupee"></i> <? if(isset($data['prprice'])) { echo $data['prprice'] ; } else { echo $data['price'];} ?></div></div>
</div>
<div class="row">
<div class="col-lg-12">
...
<? }   ?>
<?=$this->cell('Paging::createSearchPagination',[$currentpage,$total,$table,$params]);?>
