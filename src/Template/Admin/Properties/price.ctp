...
td input {margin-left: 15px}
</style>
<ol class="breadcrumb">
            <li class="breadcrumb-item">
              <a href="<?=SITE_PATH?>admin/users/welcome/">Dashboard</a>
            </li>
            <li class="breadcrumb-item active">Properties Price</li>
          </ol>
...
<div class="form-group container">
	...
	if(iterator_count($priceManagement)) { foreach($priceManagement as $key1=>$value2){
		echo "<div id='rowNum".($key1+1)."'><table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'><tr><td><label for='Unit'>Unit</label><<select name='unit_id[]' id='unit-id' required='required'>";
		...
		{ 
		echo "<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'><tr><td><label for='Unit'>Unit</label><select name='unit_id[]' id='unit-id' required>";
...
function addRow(frm) {
	rowNum ++;
	var row = '<div id="rowNum'+rowNum+'"><table class="table table-bordered" id="dataTable" width="100%" cellspacing="0"><tr><td><div class="input select required"><label for="unit-id">Unit'+rowNum+'</label><select name="unit_id[]" id="unit-id" required="required"><? foreach($unit as $key=>$value){echo "<option value=".$value['id'].">".$value['name']."</option>";}?></select></div></td><td><div class="input text required"><label for="PriceManagementPrice">Price'+rowNum+'</label><input name="price[]" maxlength="120" type="text" id="PriceManagementPrice" required="required"/></div></td><td><div class="input text required"><label for="PriceManagementSize">Size'+rowNum+'</label><input name="size[]" maxlength="120" type="text" id="PriceManagementSize" required="required"/></div></td></tr></table> <div class="small_button"><input type="button" value="Remove" onclick="removeRow('+rowNum+');"></div></div>';
