...
<?=$this->cell('Paging::ajaxpaging',[$currentpage,$total]);?>
<script>
$("ul.pagination li").click(function(){ 
var eventclass=$(this).attr("class");
if(eventclass=="disabled") { return false;}
else {
var c=$(this).attr('p'); 
var params = $(".builder_select").map(function() { return this.value;}).get().join(",");
$(".pr_con").html("loading please wait......");
$.ajax({url:"<?=SITE_PATH?>locations/searchProp/"+escape(params)+",<?=$locid?>,<?=$secid?>/"+escape(c),dataType:"html",success:function(result){
$(".pr_con").html(result);
}});
}
});
