    ...
    public function view(){
	   ...
	   $connection = ConnectionManager::get('default');
         ...
         $mainurl=rtrim($this->request->url,"/"); 
			...
			return $this->redirect(SITE_PATH.'sublocation/'.$location.'/'.$url.'/'); exit; }
			...
			$per_page = 6;
			$start = $page * $per_page;
			$pagedata = $connection->execute("SELECT psubtypeloc.* FROM propertype_sublocations as psubtypeloc join locations as  loc on psubtypeloc.location_id=loc.id where loc.name=:name and psubtypeloc.url=:url and psubtypeloc.status=:status",['name'=>$location,'url'=>$url,'status'=>'active'])->fetchAll('assoc');
			
			if(empty($pagedata)) {throw new NotFoundException(__('Page not found on the server'));}
			
			
$count = $connection->execute("select count(*) as total from properties where status=:status and location_id=:location_id and propertype_sublocation_id=:propertype_sublocation_id",['status'=>'active','location_id'=>$pagedata[0]['location_id'],'propertype_sublocation_id'=>$pagedata[0]['id']])->fetchAll('assoc');
			
			$property = $connection->execute("select pro.name,pro.heading3,pro.url,pro.small_description,pro.area,pro.property_image,pro.property_image_alt_tag,pro.price,pro.small_info,pro.property_subtype_id,pro.availability,pro.location_text,sec.url as sectorurl,ptype.name as ptypename,build.name as buildname,locat.name as locatname,locat.url as locaturl from properties as pro join property_types as ptype on pro.property_type_id=ptype.id  join builders as build  on pro.builder_id=build.id join locations as locat on pro.location_id=locat.id join sectors as sec on pro.sector_id=sec.id where pro.status=:status and pro.location_id=:location_id and pro.propertype_sublocation_id=:propertype_sublocation_id order by pro.builderwise_priority asc  limit $start,$per_page",['status'=>'active','location_id'=>$pagedata[0]['location_id'],'propertype_sublocation_id'=>$pagedata[0]['id']])->fetchAll('assoc');
			...
			$this->set('secid', $pagedata[0]['id']);
			$this->set('table','sublocation/'.$location.'/'.$url);
			...
			$this->set('canonical', SITE_PATH.'sublocation/'.$location.'/'.$url.'/');
			}
			else{
			$this->set('robots','<META NAME="ROBOTS" CONTENT="NOINDEX, FOLLOW">');
			$this->set('canonical',SITE_PATH.'sublocation/'.$location.'/'.$url.'/?page='.$this->request->query['page']);
			}
		
		    $this->layout='sublocation-list';
	...
	list($min,$max,$bid,$spid,$bhk,$locid,$secid)=@explode(',',$search);
	...
	if($secid!=0 and $secid!=''){ $sql.=" and  properties.propertype_sublocation_id='".$secid."'";}
